...
/**
 * Channel 엔티티의 영속성을 처리하기 위한 Repository 인터페이스입니다. 공통 CRUD 기능은 BaseRepository로부터 상속받고, Channel 고유의 조회
 * 기능을 추가로 정의합니다.
 */
public interface ChannelRepository extends BaseRepository<Channel, UUID> {

  /**
   * 채널 이름(channelName)으로 채널을 조회합니다. 채널 이름은 고유한 값으로 간주됩니다.
   *
  ...
  List<Channel> findAllChannelsBySettings(String channelName, String description);
   ...
   *
   * @param name 조회할 채널의 이름
   * @return 채널이 존재하면 Optional<Channel>로 감싸서 반환하고, 없으면 Optional.empty()를 반환합니다. **위 조건에서
   * 논리적(!isDelete) 삭제가 되지않은 데이터만 반환합니다.
   */
   ...
   *
   * @param name 확인할 채널의 이름
   * @return 채널 이름이 존재하면 true, 아니면 false **위 조건에서 논리적(!isDelete) 삭제가 되지않은 데이터만 반환합니다.
   */

...
public interface ParticipationService extends BaseService<Participation, ParticipationDualKey> {
  ...
  Participation joinChannel(ParticipationRequestDTO requestDTO, UUID userId);
   ...
   * @return 채널의 마지막 사용자가 나갔으면 true아니면 False ** 추후App에서 Channel에게 삭제 명령을 내리게 하기 위해 boolean을 리턴함**
   */
...
//  boolean isOwner(UUID channelId, UUID userId);
  ...
  ParticipationResponseDTO findByTempId(UUID tempId);
}